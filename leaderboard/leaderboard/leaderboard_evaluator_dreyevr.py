...
This file is modified to work with DReyeVR ego vehicles 
...
class LeaderboardEvaluator(object):
    ...
    def _set_dreyevr_ego_vehicle(self):
        """
        Spawn or update the ego vehicles
        """
        dreyevr_ego_vehicle = find_ego_vehicle(CarlaDataProvider.get_world())
        self.ego_vehicles.append(dreyevr_ego_vehicle)
        # for i, _ in enumerate(self.ego_vehicles):
        #     self.ego_vehicles[i].set_transform(ego_vehicles[i].transform)

        # TODO : sync state -- this only works if we've already started the replay
        CarlaDataProvider.get_world().tick()
    ...
    def _load_and_run_scenario(self, args, config):
        """
        This used to Load and run the scenario given by config.

        In the dreyevr case, we don't want to run the scenario, but we want the plan from inside it.
        ...
        try:
            ...
            # Load scenario and run it -- TODO : don't care about running this
            if args.record:
